...
csv_path1 = Path(__file__).parents[1] / 'Cleaned_Datasets/articles.csv'
csv_path2 = Path(__file__).parents[1] / 'Cleaned_Datasets/cnbcarticles.csv'
csv_path3 = Path(__file__).parents[1] / 'Cleaned_Datasets/cnnarticles.csv'
csv_path4 = Path(__file__).parents[1] / 'Cleaned_Datasets/datatrain.csv'


st.title("News Document Retrieval")
...
with st.spinner('Processing your query...'):
    querytext = extract_data(query)
    prediction_text = str(querytext)
    ...
    tfidf = tf.fit_transform(cnnarticles["Article text"], )
    tfidf_prediction_text = tf.transform([prediction_text])
    similarity_measure = cosine_similarity(tfidf, tfidf_prediction_text)
    print("similarity measure: \n", similarity_measure)

        
    similarity_scores = {label: {"sum": 0, "count": 0} for label in labels} 
    for i in range(len(similarity_measure)):
        similarity_scores[cnnarticles["Headline"][i]]["sum"] += similarity_measure[i]
        similarity_scores[cnnarticles["Headline"][i]]["count"] += 1
